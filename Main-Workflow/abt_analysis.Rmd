---
title: "ABT Dataset - Tuned Parameters and Differential Expression Analysis"
...
```
```{r}
vis_a <- data.frame(plot_data[0:623,],a_or_b="a")
vis_b <- data.frame(plot_data[624:1287,],a_or_b="b")
vis <- rbind(vis_a,vis_b)
vis1 <- (plot_data[,1] + plot_data[,2]) / 2
vis <- data.frame(vis,average=vis1)
ggplot(vis,aes(color,average,color=a_or_b)) + geom_point() + xlab("Cluster") + ylab("UMAP") +
  ggtitle("Scatter Plot")
```

```{r}
print("For A Sample:")
table(sce_plot$Sample_ClusterIDs[0:623])
```

```{r}
print("For B Sample:")
table(sce_plot$Sample_ClusterIDs[624:1287])
```
...
write.csv(DE_genes_all$genes, 
          file = file.path("C:/Users/Asus/Desktop/last_analysis/single-cell-project","genes_ABT.csv"),
          quote = FALSE)
```
...
marker_genes = c("PFN1", "HOMEZ", "FTH1")
